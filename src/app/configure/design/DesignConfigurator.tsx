...
function DesignConfigurator({ configId, imageUrl, imageDimensions}: DesignConfiguratorProps) {
  ...
  async function saveConfiguration() {
    ...
    try {
      ...
      userImage.crossOrigin = "anonymouse";
  ...
  return (
    <div className='relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20 '>
      <div ref={containerRef} className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed p-12 text-center  focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
        <div className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831] ">
          <AspectRatio ref={phoneCaseRef} ratio={896 / 1831} className='pointer-events-none relative z-50 aspect-[896/1831] w-full'>
            <NextImage fill alt="phone image" src="/phone-template.png" className='pointer-events-none relative z-50 ' />
          </AspectRatio>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent>
                      {MODELS.options.map((model) => (
                        <DropdownMenuItem 
                        onClick={() => setOptions((prevState) => ({...prevState, model}))}
                        key={model.label} className={cn("w-full flex text-sm gap-1 items-center p-1.5  cursor-default hover:bg-zinc-100", {
                        ...
                        <div className="mt-3 space-y-4">
                          {
                            selectableOptions.map((option) => (
                              <RadioGroup.Option
                               key={option.value}
                               value={option}
                               className={({active, checked}) => cn("relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:outline-none ring-0 focus:ring-0 outline-none sk:flex sm:justify-between", {
